...
const IssueOne = () => {
    ...
    const onIssue = () => {
        let fields = credentialFields.filter(field => field.key.trim().length > 0 && field.value.trim().length > 0);
        let credential = {};
        ...
        <Box sx={{ marginInline: "5vw", display: "flex", justifyContent: "center" }}>
            <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                    <TextField value={holderIdentifier} onChange={(event) => setHolderIdentifier(event.target.value)} 
                        id="holder" label="Holder's identifier" variant="filled" fullWidth required/>
                </Grid>
                <Grid item xs={12} sm={6}>
                    <TextField value={issuerPrivateKey} onChange={(event) => setIssuePrivateKey(event.target.value)} 
                        id="issuer" label="Issuer's Private Key" variant="filled" type="password" fullWidth required/>
                </Grid>
                <Grid item xs={12}>
                {
                    credentialFields.map((obj, index) => {
                        return(
                            <Box key={index} sx={{ display: "flex", gap: "10px", alignItems: "center", minHeight: "75px", minWidth: "70vw", maxWidth: "90vw" }}>
                                <TextField label="Field's name" value={obj.key} onChange={(event) => onFieldKeyChange(index, event.target.value)}
                                    sx={{ padding: "0px !important;", flex: 1, alignSelf: "flex-start" }}
                                    error={obj.error} helperText={obj.helperText}/>
                                <TextField label="Value" value={obj.value} onChange={(event) => onFieldValueChange(index, event.target.value)}
                                    sx={{ padding: "0px !important;", flex: 1, alignSelf: "flex-start" }}/>
                ...
                <Grid item xs={12}>
                    <Button variant="contained" sx={{ width: "100%", padding: "10px 0px"}} onClick={onIssue}>Issue</Button>
                </Grid>
            </Grid>
        </Box>
    );
