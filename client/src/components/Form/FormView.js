...
const FormView = ({ form, submitForm }) => {
    ...
    const onSubmit = async () => {
        ...
        if (validateCredential()) {
            ...
            if (decrypted !== "false") {
                ...
                setAccepted(submissionResponse);
            }
            else {
    ...
    return (
        <Box sx={{ marginInline: "5vw", display: "flex", justifyContent: "center" }}>
            <Grid container spacing={2}>
                <Grid item xs={12}>
                    <TextField value={form.verifierPublicKey} 
                               id="issuer"
                ...
                {!unencrypted && (
                    <Grid item xs={12}>
                        <TextField value={holderPrivateKey} 
                                onChange={(event) => setHolderPrivateKey(event.target.value)} 
                                id="issuer" 
                ...
                {cred !== "" && (
                    <Grid item xs={12}>
                        <Typography variant="subtitle">Selected credential id: {cred.id}</Typography>
                    </Grid>
                )}
                {json !== null && (
                    <Grid item xs={12}>
